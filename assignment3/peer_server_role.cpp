...
#include "P2P_HelloWorld_i.h"         // impl header
#include "Peer.h"                     // helper class

// main thread function
void *Peer::server_role (void *arg)
{
  try
    {
      ...
      CORBA::ORB_var orb = CORBA::ORB_init (peer->argc (), 
                                            peer->argv (),
                                            "server_role");
      ACE_DEBUG ((LM_DEBUG,
      ...
      // parse the arguments
      ACE_DEBUG ((LM_DEBUG,
                  "Peer main: parse the args\n"));
      if (peer->parse_args () == -1) {
